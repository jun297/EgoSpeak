...
def main(cfg):
    ...
    print(cfg)
    print(f"cfg['SOLVER']['NUM_EPOCHS']: {cfg['SOLVER']['NUM_EPOCHS']}")
    print(f"cfg['MODEL']['CHECKPOINT']: {cfg['MODEL']['CHECKPOINT']}")

    # Build data loaders
    data_loaders = {phase: build_data_loader(cfg, phase) for phase in cfg.SOLVER.PHASES}
    ...
    wandb.finish() if cfg.USE_WANDB else None
