...
def batch_inference(cfg, model, device, logger, data_loader):
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(pbar, start=1):
            ...
            for bs, (session, query_indices, num_frames) in enumerate(zip(*data[-3:])):
                # data[-3] is session, data[-2] is work_indices, data[-1] is num_frames
                ...
                # cfg.MODEL.LSTR.WORK_MEMORY_LENGTH: 40
                # cfg.MODEL.LSTR.WORK_MEMORY_SAMPLE_RATE: 1
                # cfg.MODEL.LSTR.ANTICIPATION_NUM_SAMPLES: 10
                # query_indices.shape: torch.Size([cfg.MODEL.LSTR.WORK_MEMORY_LENGTH])
                # query_indices at first: [0, 1, 2, 3, ..., 39]
                # At first, go into this if statement
                if query_indices[0] in torch.arange(0, cfg.MODEL.LSTR.WORK_MEMORY_SAMPLE_RATE):
                    if cfg.MODEL.LSTR.ANTICIPATION_NUM_SAMPLES > 0:
                        for t_a in range(0, cfg.MODEL.LSTR.ANTICIPATION_NUM_SAMPLES):
                            # at first, full_indices indicate the range:
                            # work_memory + t_a (anticiptaion timestep, from 0 to 9)
                            ...
                            # then assgin the currnet logits + anticipated logits to the pred_scores
    ...
    num_frames = np.concatenate(list(gt_targets.values()), axis=0).shape[0]
    logger.info(f'\nProcessed {num_frames} frames in {time_taken:.1f} seconds ({num_frames / time_taken :.1f} FPS)')
    logger.info('\n=============')
    # Compute results
    if cfg.MODEL.LSTR.ANTICIPATION_NUM_SAMPLES > 0:
        maps_list = []
        ...
        for t_a in range(0, cfg.MODEL.LSTR.ANTICIPATION_NUM_SAMPLES):
            ...
            anticipation_time=(t_a + 1) / cfg.DATA.FPS * cfg.MODEL.LSTR.ANTICIPATION_SAMPLE_RATE
            
            for class_name, ap in result['per_class_AP'].items():
                if class_name not in per_class_list:
                    per_class_list[class_name] = []
                per_class_list[class_name].append(ap)
                logger.info(f'Action anticipation ({anticipation_time:.2f}s) perframe AP of {class_name}: {ap:.5f}')
        ...
        for class_name, ap_list in per_class_list.items():
            logger.info(f'Action anticipation (mean) perframe AP of {class_name}: {np.mean(ap_list):.5f}')
        
        # EDIT: save predictions!
        for session in pred_scores.keys():
            anticipation_score = pred_scores[session]
            anticipation_dir = os.path.join(cfg.OUTPUT_DIR, 'pred')
            if not os.path.exists(anticipation_dir):
                os.makedirs(anticipation_dir)
            
            np.save(osp.join(anticipation_dir, session+'.npy'), anticipation_score)
    ...
    if cfg.DATA.DATA_NAME.startswith('EK'):
        try:
            from external.rulstm.RULSTM.utils import (
                get_marginal_indexes, marginalize, softmax,
                topk_accuracy_multiple_timesteps,
                topk_recall_multiple_timesteps, tta)
        except:
...
def do_perframe_det_batch_inference(cfg, model, device, logger):
    ...
    logger.info(f"phase: test) Inference on test_dataloader")
    start = time.time()
    batch_inference(cfg, model, device, logger, data_loader)
    end = time.time() - start
    
    logger.info(f"Elapsed time for function batch_inference: {end:.2f}s")
    
    # data_loader = torch.utils.data.DataLoader(
    #     dataset=build_dataset(cfg, phase='train', tag='BatchInference'),
    #     batch_size=cfg.DATA_LOADER.BATCH_SIZE,
    #     num_workers=cfg.DATA_LOADER.NUM_WORKERS,
    #     pin_memory=cfg.DATA_LOADER.PIN_MEMORY,
    # )
    
    # logger.info(f"***** phase: train")
    # batch_inference(cfg, model, device, logger, data_loader)
