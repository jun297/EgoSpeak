...
def load_cfg():
    ...
    for i in range(0, len(args.opts), 2):
        key = args.opts[i]
        value = args.opts[i+1]
        if key == 'MODEL.LSTR.LONG_MEMORY_SECONDS' or key == 'MODEL.LSTR.WORK_MEMORY_SECONDS':
            args.opts[i+1] = int(value)
            print(f'(key, value): ({key}, {value}) is converted to integer type')
    ...
    # config_file is in cfg.OUTPUT_DIR or args.config_file
    return cfg
