...
from rekognition_online_action_detection.utils.ek_utils import (
    action_to_noun_map, action_to_verb_map)
...
@registry.register('LSTRYTConv')
@registry.register('LSTREGO4D')
@registry.register('LSTREasyCom')
...
@registry.register('LSTRBatchInferenceEGO4D')
@registry.register('LSTRBatchInferenceEasyCom')
...
class LSTRBatchInferenceDataLayer(data.Dataset):
    ...
    def __init__(self, cfg, phase='test'):
        ...
        if phase != 'test':
            warnings.warn('originally, phase must be `test` for batch inference, got {}'.format(phase))
