        ...
        if self.long_enabled:
            ...
            for i, param in enumerate(cfg.MODEL.LSTR.ENC_MODULE):
                if param[0] != -1:
                    ...
                    # in TeSTra, not like LSTR, gets atteyntion_type, decay_alpha as parameter
                    # however, the parameter does not change anything from LSTR
    ...
    def forward(self, visual_inputs, motion_inputs, object_inputs, memory_key_padding_mask=None):
        ...
        if self.work_enabled:
            ...
            if visual_inputs.ndim == 5:
                ...
                object_inputs_avg = object_inputs[:, self.long_memory_num_samples:].mean((2, 3))
            else:
                visual_inputs_avg = visual_inputs[:, self.long_memory_num_samples:]
                motion_inputs_avg = motion_inputs[:, self.long_memory_num_samples:]
                object_inputs_avg = object_inputs[:, self.long_memory_num_samples:]
        ...
        # score (before transpose) [len, batch_size, num_classes]
        # self.classifier: nn.Linear(self.d_model, self.num_classes)
