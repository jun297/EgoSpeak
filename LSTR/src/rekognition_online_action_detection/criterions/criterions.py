...
class MultipCrossEntropyLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        # LSTR transform input from [16,50,3] to [800,3]
...
@CRITERIONS.register('MCE_MiniROAD')
class MiniROADMultipCrossEntropyLoss(nn.Module):

    def __init__(self, reduction='mean', ignore_index=-100):
        super(MiniROADMultipCrossEntropyLoss, self).__init__()
    ...
    def forward(self, input, target):
        logsoftmax = nn.LogSoftmax(dim=-1).to(input.device)
        output = torch.sum(-F.normalize(target) * logsoftmax(input), dim=1) # B
        if self.reduction == 'mean':
...
@CRITERIONS.register('MCE_ANT')
class AntMultipCrossEntropyLoss(nn.Module):

    def __init__(self, reduction='mean', ignore_index=-100):
        super(AntMultipCrossEntropyLoss, self).__init__()

        self.reduction = reduction
        self.ignore_index = ignore_index

    # only ant_target loss
    def forward(self, input, target):
        logsoftmax = nn.LogSoftmax(dim=-1).to(input.device)
        ...
        if self.ignore_index >= 0:
            notice_index = [i for i in range(target.shape[-1]) if i != self.ignore_index]
            import pdb; pdb.set_trace()
            # input dim: [16, 128, 10, 3]
            # target.shape: [16, 10, 3]
            output = torch.sum()
